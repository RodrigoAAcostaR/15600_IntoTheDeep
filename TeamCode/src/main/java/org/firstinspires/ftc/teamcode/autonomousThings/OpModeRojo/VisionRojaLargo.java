package org.firstinspires.ftc.teamcode.autonomousThings.OpModeRojo;
...
@Autonomous
public class VisionRojaLargo extends CommandOpMode {
        ...
        while(opModeInInit()) {
            ...
            if (getDistance(width) < 100) {
                if (cX < 300 && cX > 1) {
                    ...
                    CommandScheduler.getInstance().schedule(new RojoIzqLrg(drive, intake, elevador));
                }
                else if (cX > 300 && cX < 600) {
                    telemetry.addData("Posicion", "En medio");
                    CommandScheduler.getInstance().schedule(new RojoMidLrg(drive, intake, elevador));
                }
            }
            else{
                telemetry.addData("Posicion", "Lado derecho");
                CommandScheduler.getInstance().schedule(new RojoDerLrg(drive, intake, elevador));
            }
            telemetry.update();
        }
        telemetry.update();
